...
export default async function BrokerPage() {
  ...
  return (
    <PageShell
      ...
      description="Renewal and claims summary tailored to broker and book-management workflows."
    >
      <ProtectedRoute
        title="Broker and agent dashboard"
        description="Broker-facing client data is available only to signed-in users with the right role."
        allowedRoles={["broker", "admin"]}
      >
        <div className="mb-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_0.95fr]">
          ...
          <article className="broker-focus-panel rounded-[1.8rem] border border-stone-200 bg-[linear-gradient(145deg,rgba(244,248,251,0.94),rgba(221,235,245,0.94))] p-6 shadow-[0_12px_40px_rgba(15,23,42,0.08)]">
            <p className="section-eyebrow">Retention Focus</p>
            <h2 className="section-title">Broker priorities this week</h2>
            {brokerData.accounts.length === 0 ? (
              <div className="mt-5 space-y-4">
                <article className="broker-focus-card rounded-[1.4rem] border border-white/80 bg-white/80 p-4">
